...
{
    public Vector3 fucking_velocity;
    ...
    public bool clam_horizontal_rotation;
    public bool enableCameraMovement = true;
    public bool turn_head;
    ...
    #endregion
    public bool GetDidJump() { return didJump; }
    private void Awake()
    {
    ...
    private void Start()
    {
        ...
        camera_anim = playerCamera.transform.parent.GetComponent<Animator>();
        AnimateCamera = true;
        ...
        if (autoCrosshair || drawStaminaMeter)
        {
            ...
            Canvas canvas = SingleUtils.canvas.GetComponent<Canvas>();
    ...
    public float start_clamp_horizotal_angles;




    void FuckingHardDebug()
    {
        //SingleUtils.no_control.debug();
        if (Input.GetKeyDown(KeyCode.P))
        {
            ControllerPause(false);
            CameraPause();
        }
        if (Input.GetKeyDown(KeyCode.M)) Fly();

        if (Input.GetKeyDown(KeyCode.H)) Health.EarthQuake();
        if (Input.GetKeyDown(KeyCode.K)) Health.BulletHit();
    }
    public bool addFuckingAngles;
    public Vector3 fucingAngles;
    ...
    public void ReloadCamera()
    {
        ...
        if (addFuckingAngles)
        {
            Vector3 n = fucingAngles - originalRotation;
            followAngles = new Vector3(followAngles.x, n.y, n.z);
            targetAngles = new Vector3(followAngles.x, n.y, n.z);
            addFuckingAngles = false;
        }
        // Debug.Log("follow angles: " + followAngles + "  target angles: " + targetAngles);
        playerCamera.transform.localRotation = Quaternion.Euler(-followAngles.x + originalRotation.x, 0, targetAngles.z);

        transform.rotation = Quaternion.Euler(0, followAngles.y + originalRotation.y, 0);
    }
    private void Update()
    {
        ...
        if (enableCameraMovement)
        {
            ...
            if (clam_horizontal_rotation)
                targetAngles.y = Mathf.Clamp(targetAngles.y, start_clamp_horizotal_angles - 90, start_clamp_horizotal_angles + 90);
            ...
            if (turn_head)
            {
                Vector2 _lens_shift = Vector2.zero;
                bool q_key = Input.GetKey(KeyCode.Q);
                bool e_key = Input.GetKey(KeyCode.E);
                float z_angle = 0;
                if (q_key && e_key)
                {
                    z_angle = 0;
                    _lens_shift = Vector2.zero;
                }
                else if (q_key && !e_key)
                {
                    z_angle = 35;
                    _lens_shift = new Vector2(-0.15f, 0);
                }
                else if (!q_key && e_key)
                {
                    z_angle = -35;
                ...
                float vel = 0;
                playerCamera.transform.localPosition = new Vector3(Mathf.SmoothDamp(playerCamera.transform.localPosition.x, _lens_shift.x * 2, ref vel, Time.deltaTime * 2.5f), 0, 0);
                playerCamera.lensShift = Vector2.SmoothDamp(playerCamera.lensShift, _lens_shift, ref velocity, Time.deltaTime * 2.5f);
                //playerCamera.lensShift = Vector2.Lerp(Vector2.zero, _lens_shift, Time.deltaTime);

                targetAngles.z = Mathf.SmoothDamp(targetAngles.z, z_angle, ref vel, Time.deltaTime * 4);
            }
        ...
        if (_crouchModifiers.useCrouch)
        {
            ...
            {
                float ray_module = _crouchModifiers.colliderHeight - capsule.height / 2;
                RaycastHit hit;
                //Physics.SphereCast(SingleUtils.player.position,0.3f, SingleUtils.player.up, out hit, ray_module);
                Physics.Raycast(SingleUtils.player.position, SingleUtils.player.up, out hit, ray_module);
    ...
    void Ch(float y)
    {
        if (Mathf.Abs(y) > 20) Debug.Break();
    }
    public bool fly;
    bool fucking_gravity;
    public void SetFuckingGravity()
    {
        fucking_gravity = !fucking_gravity;
    }
    public void Fly()
    {
        ...
        ControllerPause(false);
        fly = !fly;
        fps_Rigidbody.isKinematic = !fps_Rigidbody.isKinematic;
        fps_Rigidbody.useGravity = !fps_Rigidbody.useGravity;
        //fucking_velocity.y = 
    }

    public Vector2 GetInput() { return inputXY; }


    bool hold_lay = false;

    public Transform ground;
    RaycastHit groun_hit;


    private void FixedUpdate()
    {

        if (Physics.Raycast(SingleUtils.player.position, Vector3.down, out groun_hit, 1 << 14))
        {
            ground = groun_hit.transform;
        }
        ...
        if (useStamina)
        {
            ...
            if (AnimateCamera)
            {
                ...
                camera_anim.SetFloat("StaminaFor", 0.33f + 0.66f * (SingleUtils.person.staminaInternal * 2 / 100));
                camera_anim.SetFloat("Stamina", 1 + (staminaLevel - staminaInternal) / staminaLevel * 3);
            }
        ...
        if (!controllerPauseState)
        {
            ...
            if (!didJump)
            {
                if (IsGrounded && jumpInput && jumpPowerInternal > 0)
                {

                    staminaInternal -= 3;
                    Vector3 closest = Vector3.zero;
                    if (SingleUtils.border)
                    {
                        closest = SingleUtils.border.GetComponent<Collider>().ClosestPoint(SingleUtils.player.position);

                    }
                    float angle_to_border = Vector3.Angle(SingleUtils.player.forward, new Vector3(closest.x, SingleUtils.player.position.y, closest.z) - SingleUtils.player.position);


                    if (SingleUtils.on_trigger_border && angle_to_border <= 30)
                    {

                        jumpInput = false;
                        if (SingleUtils.can_over)
                        {
                            if (Input.GetAxis("Vertical") > 0.5f)
                            {
                                SingleUtils.grab_trigger.GrabingForBorder(SingleUtils.border);
                                //playerCanMove = false;
                                ControllerPause(false);
                                CameraPause();
                            }
                            ...
                            {
                                if (SingleUtils.can_on)
                                {
                                    SingleUtils.grab_trigger.SetBorderForOnePoint();
                                    SingleUtils.grab_trigger.GrabingForBorder(SingleUtils.border);
                                    //playerCanMove = false;
                                    ControllerPause(false);
                                    CameraPause();
        ...
        if (advanced.maxSlopeAngle > 0)
        {
            float angle = Vector3.Angle(SingleUtils.player.up,advanced.curntGroundNormal);
           
            /*if(angle>advanced.maxSlopeAngle+10&&!SingleUtils.no_control.fucking_falling){
        ...
        {
            if (!fucking_gravity)
                fps_Rigidbody.velocity = fucking_velocity;
        ...
        if (advanced.FOVKickAmount > 0)
        {
            if (isSprinting && !isCrouching && playerCamera.fieldOfView != (baseCamFOV + (advanced.FOVKickAmount * 2) - 0.01f))
            {
                ...
                {
                    if (SingleUtils.gun_state == GunState.aim)
                        playerCamera.fieldOfView = Mathf.SmoothDamp(playerCamera.fieldOfView, baseCamFOV / 4 * 3 + (advanced.FOVKickAmount * 2), ref advanced.fovRef, advanced.changeTime / 2);
            ...
            {
                if (SingleUtils.gun_state == GunState.aim)
                    playerCamera.fieldOfView = Mathf.SmoothDamp(playerCamera.fieldOfView, baseCamFOV / 4 * 3, ref advanced.fovRef, advanced.changeTime / 4f);
                else
        ...
        if (_crouchModifiers.useCrouch && !controllerPauseState)
        {
            ...
            if (isCrouching)
            {
                ...
                {
                    head_offset = _crouchModifiers.colliderHeight / 3f;
                }
                else
                {
                    if (hold_lay)
                    {
                        RaycastHit hit;
                        float dist = head_offset = _crouchModifiers.colliderHeight / 1.5f - capsule.height / 2;
                        Physics.Raycast(SingleUtils.player.position, SingleUtils.player.up, out hit, dist);
                        if (hit.collider == null)
                        {
                            hold_lay = false;
                        }
                    ...
                    {
                        head_offset = _crouchModifiers.colliderHeight / 1.5f;
                    }

                }


                capsule.height = Mathf.MoveTowards(capsule.height, head_offset, 5 * Time.deltaTime);
    ...
    public void CameraPause()
    {
        ...
        {
            Cursor.lockState = enableCameraMovement ? CursorLockMode.None : CursorLockMode.Locked;
            Cursor.visible = enableCameraMovement;
        }
    }
    public void ControllerPause()
    {
        fps_Rigidbody.isKinematic = false;
        fps_Rigidbody.useGravity = true;
        controllerPauseState = false;
        fucking_gravity = false;
        if (!camera_anim.isActiveAndEnabled)
            camera_anim.enabled = true;
        useHeadbob = true;
        if (!useStamina) useStamina = true;

    }
    public void ControllerPause(bool gravity)
    {
        //fps_Rigidbody.IsKinematic = !fps_Rigidbody.IsKinematic;
        if (gravity)
        {
            fucking_gravity = true;
            useStamina = false;
        ...
        else
        {
            ...
            fps_Rigidbody.isKinematic = true;
        }
        controllerPauseState = true;
    ...
    private void OnCollisionEnter(Collision CollisionData)
    {
        ...
        for (int i = 0; i < CollisionData.contactCount; i++)
        {
            ...
            //offset_height = ((capsule.height/2) - capsule.radius*0.95f);
            offset_height = 0.8f;
            if (CollisionData.GetContact(i).point.y < transform.position.y - offset_height)
            {
...
public class FPAIO_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        t.turn_head = EditorGUILayout.ToggleLeft(new GUIContent("turn", "tunr"), t.turn_head);
        ...
        t.fucking_velocity = EditorGUILayout.Vector3Field(new GUIContent("ds", "df"), t.fucking_velocity);
        EditorGUILayout.Toggle(new GUIContent("Did jump", "did jump"), t.GetDidJump());
        EditorGUILayout.Toggle(new GUIContent("Is grounded", "Fucking ground"), t.IsGrounded);
