	...
	IEnumerator Spawn(int _redniBroj){
		...
		if(currentGun){
			if(currentGun.name.Contains("Gun")){
                ...
                gun_transform.GetChild(_redniBroj).gameObject.SetActive(true);
                currentGun = gun_transform.GetChild(_redniBroj).gameObject;
                currentGun.transform.parent = gun_transform;
                currentGun.transform.SetAsFirstSibling();
